...
/********    本文件：主ui的交互界面（主要包含主窗口相关（控件）功能的对应接口实现） *********/
...
HairInterface::HairInterface(Ui::MainWindow *ui)
{
    ...
    //1.初始化一些 指针参数
    m_ui = ui;
    m_glWidget = NULL;//绘制窗口指针
    m_hairObject = NULL;//发型对象 指针
    m_mesh = NULL;//网格指针 （setMesh函数的参数）
    //2.交互信号、槽
...
void HairInterface::connectUserInputs()
{
    ...
    //按钮   功能接口 buttons(暂停按钮、重新仿真按钮、场景编辑按钮)
    ...
    connect(m_ui->right_hair, SIGNAL(pressed()), this, SLOT(right_genhair()));//右侧头发生成按钮
    connect(m_ui->left_hair, SIGNAL(pressed()), this, SLOT(left_genhair()));//左侧头发生成按钮
}

//设置m_glWidget仿真模型窗口（glwidget.cpp里调用这个函数，给用户和ui交互的窗口上设置 glwidget窗口）
void HairInterface::setGLWidget(GLWidget *glWidget)
{
    m_glWidget = glWidget;
}

//mesh网格模型，比如把头部obj模型文件加载进来（在glwidget.cpp里的初始化函数里调用initSimulation函数，initSimulation里调用此函数）
void HairInterface::setMesh(ObjMesh *mesh)
{
    m_mesh = mesh;
}

//对（仿真模型）对象　的各个参数值的初始化设置（各个模块包含的功能对应文本框以及滑动条的值的设置）
...
//更新状态label标签的状态【引导头发(头发方向) 、渲染头发 、模拟顶点 、渲染三角形（角度）】调用了些其他模块的参数
void HairInterface::updateStatsLabel()
{
    // Update stats label.
    int numGuideHairs = m_hairObject->m_guideHairs.size();//numGuideHairs表示多少撮头发
    int numGroupHairs = m_hairObject->m_numGroupHairs;//每一撮的数量
    int numGuideVertices = m_hairObject->m_guideHairs[0]->m_vertices.size();//numGuideVertices表示第一撮的顶点
    ...
    m_ui->statsLabel->setText(
                ...
                QString::number(numGuideHairs * numGroupHairs * (numSplineVertices-1) * 2) + " rendered triangles");//镶嵌器渲染了多少三角形
}

//重新仿真    并更新状态label标签
void HairInterface::resetSimulation()
{
    //当场景编辑没有打开的时候  重设仿真   （场景编辑若打开则此按钮不响应该函数）
    if (m_glWidget->m_sceneEditor == NULL){
        m_glWidget->resetSimulation();//调用此函数进行重设仿真
        m_glWidget->forceUpdate();//如果当前状态为暂停仿真状态，则重新绘制场景。
...
void HairInterface::setHairsPerPatch(int numHairs)
{
    ...
    m_ui->sliderHairsPerPatch->setValue(numHairs);//给滑动条在设置当前值
...
void HairInterface::inputHairColorRText(QString text)
{
    ...
    double value = text.toDouble(&ok);
    if (!ok){
        value = m_hairObject->m_color.x;//x值0-255
...
void HairInterface::setHairColorR(int value)
{
    ...
    m_glWidget->forceUpdate();//如果此时仿真处于暂停 状态 ，除了仿真模块参数，如果其他模块的参数变化，绘制效果也更新
}
...
//暂停按钮　的触发槽函数，（只针对仿真模块）实现仿真模型的仿真暂停
void HairInterface::togglePaused()
{
    if (m_glWidget->isPaused())//若处于暂停
    {
        m_glWidget->unpause();//打开仿真 状态
...
void HairInterface::startEditScene(){
    if (m_glWidget->m_sceneEditor == NULL){//m_sceneEditor为子窗口,若没有打开子窗口且正在仿真，触发暂停
...
void HairInterface::right_genhair(){
    m_glWidget->which_face = 1;  //1表示右侧

}

void HairInterface::left_genhair(){
    m_glWidget->which_face = -1;  //-1表示左侧
