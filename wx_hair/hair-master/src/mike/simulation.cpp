...
#include "mike/hair.h"
#include "glwidget.h"
//宏定义一些仿真系数（比如：STIFFNESS刚度、FRICTION摩擦、REPULSION排斥力等）
#define G   -29.8f
#define B   0.35f
#define MASS 1.0f
...
Simulation::Simulation(GLWidget *widget, ObjMesh *mesh, Simulation *_oldSim)
{
    m_time = 0;
    m_widget = widget;
    m_mesh = mesh;//对应的obj模型文件
    m_xform = glm::mat4(1.0);//用于存贮更新（叠加平移旋转效果）后的modle矩阵
    m_fluidGrid = std::map<grid_loc, fluid>();
    m_headMoving = false;
    //加载相应的参数
    if (_oldSim == NULL){//若没有上个（刚运行），加载初始参数
        m_windDir = glm::vec3(1, 0, 0);//初始化：分量分别表示 右  上  重力
        m_windMagnitude = 0.0f;//风力0级
        m_friction = FRICTION;
        m_stiffness = STIFFNESS;//初始化设置为0
    } else {//如果有上个（已经设置过值了），加载上一个参数
...
void Simulation::simulate(HairObject *_object)
{
    ...
    calculateExternalForces(_object);//１．计算外部受到的力
    
    if (m_widget->useFrictionSim){//如果使用摩擦（复选框）
        
        calculateFluidGrid(_object);//计算流体网格
        //            cout << "1: " << t.restart() << " ms"<< endl;
        
        calculateFrictionAndRepulsion(_object);//计算摩擦和排斥力
        //            cout << "2: " << t.restart() << " ms"<< endl;
    }
    
    particleSimulation(_object);//进行粒子仿真（对每一根头发顶点数据结构体变量更新值）
    updateHairPosition(_object);//更新当前头发位置到prevpos
    
}
//更新每根头发位置
void Simulation::updateHairPosition(HairObject *object)
{
    for (int i = 0; i < object->m_guideHairs.size(); ++i)//遍历每一撮头发（其实就是每个网格三角形里的头发）
    {
        for (int j = 0; j < object->m_guideHairs.at(j)->m_vertices.size(); ++j)//遍历每根头发顶点数据
        {
            //  等式右边：  初始坐标×变换（平移旋转）后的矩阵m_xform
...
void Simulation::updatePosition(HairObject *object, glm::vec3 xform)
{
    ...
    m_xform = glm::translate(m_xform, xform);//函数的作用：创建并返回一个平移矩阵
    //参数：第一个参数是目标矩阵(要处理的矩阵,或之前的model矩阵)，第二个参数是平移的方向向量（要对第一个参数进行叠加效果的矩阵）
}
...
{
    updateHairPosition(object);
    m_xform = glm::rotate(m_xform, angle, axis);//创建并返回一个旋转矩阵
    //第一个参数是目标矩阵(要处理的矩阵,或之前的model矩阵) 第二个参数是旋转角度（弧度），第三个参数是旋转轴
}

// 针对模拟中包含的每个外力，计算每个关节的力
...
// 将头发转化为流体（计算流体网格）Convert the hair to a fluid
...
{
    //  start comment here to undo threading在此处开始注释以撤消线程
...
void Simulation::integrate(HairObject *_object)
{
    for (int i = 0; i < _object->m_guideHairs.size(); i++)
    {
        ...
        for (int j = 1; j < numVerts; j++){
            ...
            // 将前一个顶点视为摆轴，因此杆长是两个顶点之间的长度　Treat previous vertex at pendulum pivot, so rod length is length between two vertices
            glm::vec3 rodVector = vert->position - pivotVert->position;
            
            // 杆长对于惯性矩很有用　　Rod length is useful for moment of inertia
            float rodLength = glm::length(rodVector);
            
            // 点群的惯性矩为mR^2　　　Moment of inertia for point mass is mR^2
            float I = MASS * rodLength * rodLength;
            
            // 变量更新　　　Variables to update
...
//(粒子仿真)更新头发顶点数据结构体里的变量值
void Simulation::particleSimulation(HairObject *obj)
{
    
    
    for (int i = 0; i < obj->m_guideHairs.size(); i++)//处理每一撮头发
    {
        float numVerts = obj->m_guideHairs.at(i)->m_vertices.size();//得到当前这撮头发的顶点数据的大小
        
        obj->m_guideHairs.at(i)->m_vertices.at(0)->tempPos = obj->m_guideHairs.at(i)->m_vertices.at(0)->position;//当前这撮头发的顶点数据的第0个元素的position
        
        // Update Velocities
        for (int j = 1; j < numVerts; ++j)
        {
            HairVertex *h = obj->m_guideHairs.at(i)->m_vertices.at(j);//第i撮头发的第j根头发顶点数据
            HairVertex *prev = obj->m_guideHairs.at(i)->m_vertices.at(j - 1);//第i撮头发的第j根头发的前一根头发顶点数据
            
            // TODO: Precompute the mass inverse  对h这个结构体中的变量赋值
